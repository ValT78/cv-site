...
>
  ...
  class="group relative bg-white rounded-lg shadow-lg p-6 cursor-pointer
        ...
        hover:scale-105 hover:rotate-1 hover:ring-4 hover:ring-amber-400/50">

  <div class="flex items-center justify-between mb-2">
    <div class="flex flex-col items-center flex-1">
      <h3 class="text-2xl font-semibold text-gray-800 text-center">{exp.title}</h3>
      <p class="text-sm text-gray-600 text-center">{exp.dates}</p>
    </div>
      <div class="w-20 rounded-full bg-white flex items-center justify-center shadow-lg ring-2 ring-gray-800 ml-4">
        <img src={exp.companyLogo} alt={exp.title} class="w-20 min-h-10 max-h-15 rounded-full"/>
      </div>
  </div>
  <p class="text-blue-700 italic mb-4">{@html exp.teaser}</p>
    <div class="details">
  ...
  {#if isOpen}
    ...
    >
      ...
      {#each exp.details as blk, i}
        ...
        <div
          class="w-full h-1 animate-glow from-amber-400 via-white to-amber-400"
  ...
  <div class="mt-4 flex flex-wrap items-center justify-between">
    ...
    <a href={exp.githubLink} target="_blank"
       class="flex items-center gap-2 text-gray-800 hover:text-[#8c6c34] transition">
